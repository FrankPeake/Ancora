...
export default function IngredientListItem({
  ...
  const filteredResults = useMemo(() => {
    ...
    const currentInput =
      listType === "unit" ? localUnitName : localIngredientName
      ...
      {searchSelected && (
        <View style={{ alignItems: "center", marginLeft: 10 }}>
          <FlatList
            ...
            renderItem={({ item }) => (
              <Pressable onPress={() => handleOptionPress(item.name, item.id)}>
                <Text style={styles.searchOptions}>{item.name}</Text>
              </Pressable>
            )}
